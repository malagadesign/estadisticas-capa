				...
				<div class="normal-table-list mg-t-30">
					<div class="basic-tb-hd">
						...
						<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12" style="text-align: right;">
							<button type="button" class="btn btn-info waves-effect" data-toggle="modal" data-target="#myModalthree" id="botonCrearModificar" onclick="Fcrear();">Crear nuevo mercado</button>
						</div>
					</div>
...
$stmt = $mysqli->query("SELECT * FROM `mercados` WHERE `superado`=0 AND `elim`=0");
if($stmt === false) {
	...
	while ($row = $stmt->fetch_array(MYSQLI_ASSOC)) {
		...
		$Adatos[$did] = ['nombre'=>$nombre, 'habilitado'=>$habilitadoPA];
		echo "<tr style='cursor: pointer;' onclick='Fmodificar({$did});'><td>{$did}</td><td id='tdNom{$did}'>{$nombre}</td><td id='tdHab{$did}'>{$habilitado}</td></tr>";
	}
	$stmt->close();
	if ($did == 0){
		echo '<tr><td colspan="3" style="text-align: center;"><b>Sin datos</b></td></tr>';
	}
}
						...
						<div class="row">
							...
							<div class="col-lg-8 col-md-7 col-sm-7 col-xs-12">
								<div class="nk-int-st">
									...
									<input type="text" autocomplete="new-password" name="nombre" id="nombre" placeholder="Nombre descriptivo del mercado" class="form-control input-sm" onkeyUp="FverificarCaracteres(this);" onChange="FverificarCaracteres(this);">
								</div>
							</div>
						</div>
					</div>
...
function FguardarForm(){
	...
	if (nombreSiNo == ''){
		sinerrores = false;
		notifyBox('top', 'right', '', 'danger', 'animated bounceIn', 'animated fadeOutUp', 'Nombre del mercado', 'No se puede crear un mercado sin nombre', '', 4000);
	}
	if (sinerrores) {
		let AdatosSiNo = {que: "admMercados", did: didSiNo, nombre: nombreSiNo, habilitado: habilitadoSiNo};
