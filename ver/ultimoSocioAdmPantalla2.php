...
<div id="admPantalla" class="tab-pane fade">
	<div class="tab-ctn">
		...
		<p class="tab-mg-b-0">
			<table class="table table-striped table-hover table-bordered table-condensed cincopexis">
				<thead>
					<tr>
						<th rowspan="2" style="vertical-align: middle;">#</th>
						<th rowspan="2" style="vertical-align: middle;">Rubro</th>
...
echo "<th colspan='2' style='text-align: center; min-width: 270px;'>{$nombre}</th>";
}
?>
					</tr>
					<tr>
		...
		if ($AfamiliasDidRubro[$didFamilia] == $didRubro){
			foreach ($AarticulosNombre as $didArticulo=>$nombreArticulo){
				if ($AarticulosDidFamilia[$didArticulo] == $didFamilia){
					...
					echo "<tr {$none} id='idTR{$didArticulo}'><td>{$didArticulo}</td><td>{$nombreRubro}</td><td>{$nombreFamilia}</td><td>{$nombreArticulo}</td>";
...
function FcambioValor(didArticuloSiNo, didMercadoSiNo, tipoSiNo, queInput){
	...
	montoSiNo = queInput.value;
	montoSiNo = montoSiNo.replace(/[^0-9]/g, '');
	if (sinerrores) {
