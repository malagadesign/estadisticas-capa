				...
				<div class="normal-table-list mg-t-30">
					<div class="basic-tb-hd">
						...
						<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12" style="text-align: right;">
							<button type="button" class="btn btn-info waves-effect" data-toggle="modal" data-target="#myModalthree" id="botonCrearModificar" onclick="Fcrear();">Crear nuevo socio</button>
						</div>
					</div>
...
$stmt = $mysqli->query("SELECT * FROM `usuarios` WHERE `tipo`='socio' AND `superado`=0 AND `elim`=0");
if($stmt === false) {
					...
					<div class="form-group">
						<div class="row">
							<div class="col-lg-2 col-md-3 col-sm-3 col-xs-12">
								<label class="hrzn-fm">Socio</label>
							</div>
...
function FguardarForm(){
	...
	if (usuarioSiNo == ''){
		sinerrores = false;
		notifyBox('top', 'right', '', 'danger', 'animated bounceIn', 'animated fadeOutUp', 'Nombre de socio', 'No se puede crear un socio sin nombre', '', 4000);
	}
	if (mailSiNo == ''){
		sinerrores = false;
		notifyBox('top', 'right', '', 'danger', 'animated bounceIn', 'animated fadeOutUp', 'Mail', 'No se puede crear un socio sin mail', '', 4000);
	}
	...
	if (sinerrores) {
		let AdatosSiNo = {que: "admSocios", did: didSiNo, usuario: usuarioSiNo, mail: mailSiNo, habilitado: habilitadoSiNo};
